"""
GMFlow Transformer - copied from https://github.com/haofeixu/gmflow/blob/main/gmflow/transformer.py
"""
import torch
...
def split_feature(feature, num_splits=2, channel_last=False):
    if channel_last:  # [B, H, W, C]
        ...
        h_new = h // num_splits
        w_new = w // num_splits
        feature = feature.view(b, c, num_splits, h_new, num_splits, w_new
                               ).permute(0, 2, 4, 1, 3, 5).reshape(b_new, c, h_new, w_new)
    return feature


def merge_splits(feature, num_splits=2, channel_last=False):
    if channel_last:  # [B*K*K, H/K, W/K, C]
        b_new, h_new, w_new, c = feature.shape
        b = b_new // (num_splits * num_splits)
        h = h_new * num_splits
        w = w_new * num_splits
        feature = feature.view(b, num_splits, num_splits, h_new, w_new, c
                               ).permute(0, 1, 3, 2, 4, 5).reshape(b, h, w, c)
    else:  # [B*K*K, C, H/K, W/K]
        b_new, c, h_new, w_new = feature.shape
        b = b_new // (num_splits * num_splits)
        h = h_new * num_splits
        w = w_new * num_splits
        feature = feature.view(b, num_splits, num_splits, c, h_new, w_new
...
class GMFlowDecoderBlock(nn.Module):
    """
    Wrapper around GMFlow's TransformerBlock to match the interface expected by network.py.

    Original interface: blk(x, y) where x is decoder input, y is encoder output
    GMFlow interface: blk(source, target, height, width, shifted_window_attn_mask, attn_num_splits)
    """
    def __init__(self, dim=768, window_size=4, shift_size=0, img_size=16,
        ...
        # Calculate number of splits based on window_size
        self.attn_num_splits = img_size // window_size if window_size > 0 else 1

        # Use GMFlow's TransformerBlock
        ...
        self.shifted_window_attn_mask = None
        if shift_size > 0 and self.attn_num_splits > 1:
            h = w = img_size
            window_size_h = h // self.attn_num_splits
            window_size_w = w // self.attn_num_splits
            self.shifted_window_attn_mask = generate_shift_window_attn_mask(
                input_resolution=(h, w),
                window_size_h=window_size_h,
                window_size_w=window_size_w,
                shift_size_h=window_size_h // 2,
    ...
    def forward(self, x, y, return_attention=False):
        ...
        Args:
            ...
            return_attention: bool - not used, kept for interface compatibility
        Returns:
            x: [B, N, D] - updated decoder features
        """
        H = W = self.img_size

        # Move mask to correct device if needed
        attn_mask = self.shifted_window_attn_mask
        if attn_mask is not None and attn_mask.device != x.device:
            self.shifted_window_attn_mask = attn_mask.to(x.device)
            attn_mask = self.shifted_window_attn_mask
        ...
        x = self.block(
            ...
            height=H,
            width=W,
            shifted_window_attn_mask=attn_mask,
