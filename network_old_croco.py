# network_old_croco.py
# CroCo-like ViT Decoder for Cross-Modal VPR (Legacy)
import math
import torch
import numpy as np
...
from swin_transformer import CroCoDecoderBlock
...
class CrossModalVPR_Net(nn.Module):
    """CroCo-style Cross-Modal VPR Network with masked encoding and cross-attention decoding."""
    ...
    def __init__(self, args, pretrained_foundation=False, foundation_model_path=None):
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.patch_count, self.output_dim))
        nn.init.trunc_normal_(self.decoder_pos_embed, std=0.02)

        # Mask generator
        self.mask_generator = RandomMask(self.patch_count, args.croco_mask_ratio)
    ...
    def patchify(self, imgs):
        """Convert images to patches: (B, 3, H, W) -> (B, L, patch_size**2 * 3)"""
        p = 14
    ...
    def croco_encoded_mask_expension(self, visible_patches, mask, patch_B, patch_N, patch_D):
        """Fill masked positions with mask tokens."""
        full_patches = self.mask_token.expand(patch_B, patch_N, -1).clone()
        full_patches[~mask] = visible_patches.flatten(0, 1)
        full_patches = full_patches.view(patch_B, patch_N, patch_D)
        return full_patches
    ...
    def forward_model(self, x, paired_rgb=None, modality='rgb'):
        ...
        if modality == 'rgb':
            ...
            if self.training:
                # Masked encoding for both modalities
                ...
                paired_thermal_cls_attn = paired_thermal["cls_attention"].sum(dim=1)
                ...
                cls_attn_map = paired_thermal_cls_attn
                ...
                # CroCo Decoder: cross-attention between thermal and RGB
        ...
        return global_desc, patch_tokens, [recon_loss_thermal, recon_loss_rgb], mask_thermal, cls_attn_map, gem_attn_map
    ...
    def forward(self, x, flags, paired_rgb=None, return_mask=False):
        ...
        if is_rgb.any():
            global_emb, patch_rgb, _, _, cls_rgb_attn, gem_rgb_attn = self.forward_model(x[is_rgb], modality='rgb')
            if global_emb is not None:
                final_emb[is_rgb] = global_emb
            if patch_rgb is not None:
                patch_emb[is_rgb] = patch_rgb
            if cls_rgb_attn is not None:
                cls_attn_map[is_rgb] = cls_rgb_attn
            if gem_rgb_attn is not None:
                gem_attn_map[is_rgb] = gem_rgb_attn

        if (~is_rgb).any():
            global_emb, patch_thermal, recon_losses, mask, cls_thermal_attn, gem_thermal_attn = self.forward_model(x[~is_rgb], modality='thermal', paired_rgb=paired_rgb)
            if global_emb is not None:
                final_emb[~is_rgb] = global_emb
            ...
            if return_mask:
                masks[~is_rgb] = mask
            if cls_thermal_attn is not None:
                cls_attn_map[~is_rgb] = cls_thermal_attn
            if gem_thermal_attn is not None:
                gem_attn_map[~is_rgb] = gem_thermal_attn

        return final_emb, patch_emb, recon_losses, masks, cls_attn_map, gem_attn_map
