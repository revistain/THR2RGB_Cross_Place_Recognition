    ...
    def forward_features(self, x, masks=None, return_attention=False):
        ...
        if return_attention:
            if attn is not None:
                ...
                result["cls_attention"] = attn[:, :, 0, self.num_register_tokens+1:]
            
            if penultimate_attn is not None:
                result["penultimate_attention"] = penultimate_attn
                result["cls_penultimate_attention"] = penultimate_attn[:, :, 0, self.num_register_tokens+1:]
                
                if penultimate_x is not None:
                    penultimate_x_norm = self.norm(penultimate_x)
                    result["penultimate_norm_patchtokens"] = penultimate_x_norm[:, self.num_register_tokens+1:]
...
#     def prepare_tokens_with_masks(self, x, masks=None):
#         B, nc, w, h = x.shape # B, nc, w, h: 1 3 224 224
#         x = self.patch_embed(x) #x.shape: torch.Size([1, 256, 768])
#         if masks is not None:
#             x = torch.where(masks.unsqueeze(-1), self.mask_token.to(x.dtype).unsqueeze(0), x)

#         x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)  #x.shape: torch.Size([1, 257, 768])
#         x = x + self.interpolate_pos_encoding(x, w, h)
