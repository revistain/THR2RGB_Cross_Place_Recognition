...
from network import CroCoOnlyCrossDecoderBlock # 그냥 network.py꺼 쓰기로(2개 관리하기 귀찮음)
...
class CrossModalVPR_Net(nn.Module):
    def __init__(self, pretrained_foundation=False, foundation_model_path=None,
                 ...
                 use_bireconstruction=False, use_only_cross_decoder=False):
        super().__init__()
        ...
        self.use_bireconstruction = use_bireconstruction
    ...
    def _set_mask_token(self, dec_embed_dim):
        ...
        if self.use_bireconstruction:
            self.mask_token_rgb = nn.Parameter(torch.zeros(1, 1, dec_embed_dim))
            nn.init.normal_(self.mask_token_rgb, std=.02)
    ...
    def _set_prediction_head(self, dec_embed_dim, patch_size):
        ...
        if self.use_bireconstruction:
            self.prediction_head_rgb = nn.Sequential(
                nn.Linear(dec_embed_dim, patch_size**2 * 3), # 768 → 588
            )
            nn.init.normal_(self.prediction_head[0].weight, std=0.02)
            nn.init.zeros_(self.prediction_head[0].bias)
    ...
    def forward_model(self, x, aligned_rgb=None, modality='rgb', return_masked_patch=False):
        ...
        if modality == 'rgb':
            ...
            if self.training:
                if self.use_bireconstruction:
                    # Extract encoded feature from Thermal backbone
                    thermal_full = self.thermal_backbone(x)
                    thermal_full = thermal_full["x_norm_patchtokens"] 
                    
                    # aligned RGB도 feature tokens 추출하기
                    rgb_full = self.rgb_backbone(aligned_rgb)
                    rgb_full = rgb_full["x_norm_patchtokens"]
                    
                    patch_B, patch_N, patch_D = thermal_full.shape
                    
                    # Masked positions에 mask_token 삽입
                    ## Thermal
                    thermal_visible = thermal_full.clone()
                    mask_thermal = self.mask_generator(thermal_full)
                    mask_token_expanded = self.mask_token.expand(patch_B, patch_N, patch_D)
                    thermal_visible[mask_thermal] = mask_token_expanded[mask_thermal]
                    
                    ## RGB
                    rgb_visible = rgb_full.clone()
                    mask_rgb = self.mask_generator(rgb_full)
                    mask_token_expanded_rgb = self.mask_token_rgb.expand(patch_B, patch_N, patch_D)
                    rgb_visible[mask_rgb] = mask_token_expanded_rgb[mask_rgb]
                
                    # 7. Decoder Positional Encoding
                    thermal_visible = thermal_visible + self.decoder_pos_embed  # [B, 256, 768]
                    thermal_mask_dec = thermal_visible
                    rgb_visible = rgb_visible + self.decoder_pos_embed  # [B, 256, 768]
                    rgb_mask_dec = rgb_visible
                    ...
                    for blk in self.decoder_blocks:
                        thermal_mask_dec = blk(thermal_mask_dec, rgb_full_dec)
                    thermal_recon_dec = self.decoder_norm(thermal_mask_dec)
                    
                    for blk in self.decoder_blocks:
                        rgb_mask_dec = blk(rgb_mask_dec, thermal_full_dec)
                    rgb_recon_dec = self.decoder_norm(rgb_mask_dec)
                    
                    # 9. Prediction Head
                    reconstructed_patches = self.prediction_head(thermal_recon_dec)
                    target_patches = self.patchify(x)
                    
                    reconstructed_patches_rgb = self.prediction_head_rgb(rgb_recon_dec)
                    target_patches_rgb = self.patchify(aligned_rgb)
                    
                    # 10. Reconstruction loss 계산
                    recon_loss_thermal = self.reconstruction_criterion(
                        pred=reconstructed_patches, # [B, 256, 768]
                        mask=mask_thermal,              # [B, 256]
                        target=target_patches       # [B, 3, 256, 768]
                    )
                    recon_loss_rgb = self.reconstruction_criterion(
                        pred=reconstructed_patches_rgb,  # [B, 256, 768]
                        mask=mask_rgb,                   # [B, 256]
                        target=target_patches_rgb        # [B, 3, 256, 768]
                    )
                    ...
                    out = {"x_norm_patchtokens": thermal_full}
                    mask = mask_thermal
                else:
                    # 1-4. extract encoded feature from Thermal backbone
                    thermal_full = self.thermal_backbone(x)
                    thermal_full = thermal_full["x_norm_patchtokens"] 
                    ...
                    patch_B, patch_N, patch_D = thermal_full.shape
                    thermal_visible = thermal_full.clone()
                    mask = self.mask_generator(thermal_full)
                    ...
                    # 7. Decoder Positional Encoding
                    thermal_visible = thermal_visible + self.decoder_pos_embed  # [B, 256, 768]
                    thermal_full_dec = thermal_visible
                ...
                if self.use_masked_inference:
                    ...
                    patch_B, patch_N, patch_D = thermal_full.shape
                    mask = self.mask_generator(thermal_full)
                    thermal_visible = thermal_full[~mask].reshape(patch_B, -1, patch_D)
                    ...
                    # 7. Decoder Positional Encoding
                    thermal_full = thermal_visible + self.decoder_pos_embed  # [B, 256, 768]
